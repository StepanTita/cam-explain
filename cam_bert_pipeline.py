...
import numpy as np
import torch
from eraserbenchmark.rationale_benchmark.models.pipeline.evidence_classifier import train_evidence_classifier
from eraserbenchmark.rationale_benchmark.models.pipeline.evidence_identifier import train_evidence_identifier
from eraserbenchmark.rationale_benchmark.models.pipeline.evidence_token_identifier import \
    train_evidence_token_identifier
from eraserbenchmark.rationale_benchmark.models.pipeline.pipeline_utils import decode, \
    decode_evidence_tokens_and_classify
from eraserbenchmark.rationale_benchmark.models.sequence_taggers import BertTagger
from transformers import BertTokenizer

from cam_bert import (
    SpaceClassifier,
)
...
def initialize_models(params: dict, batch_first: bool, unk_token='<unk>', config=None, device='cuda'):
    assert batch_first
    ...
    if bool(params['use_evidence_sentence_identifier']):
        evidence_identifier = SpaceClassifier(bert_dir=bert_dir,
    ...
    evidence_classifier = SpaceClassifier(bert_dir=bert_dir,
                                          pad_token_id=pad_token_id,
                                          cls_token_id=cls_token_id,
                                          sep_token_id=sep_token_id,
                                          num_labels=len(evidence_classes),
    ...
    * decode first the evidence, then run classification for each split
    ...
    evidence_identifier, evidence_token_identifier, evidence_classifier, word_interner, de_interner, evidence_classes, tokenizer = \
        initialize_models(model_params,
                          ...
                          unk_token=unk_token, config=config, device=device)
    ...
    evidence_classifier, evidence_class_results = train_evidence_classifier(evidence_classifier,
                                                                            ...
                                                                            tensorize_model_inputs=True,
                                                                            token_only_evidence=(
                                                                                    evidence_token_identifier is not None))
    ...
    if evidence_identifier is not None:
        ...
        pipeline_results, train_decoded, val_decoded, test_decoded = decode_evidence_tokens_and_classify(
